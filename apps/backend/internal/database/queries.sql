...
SELECT * FROM hostels WHERE neighborhood_id = @neighborhood_id AND estimated_price_range <= @max_price AND estimated_price_range >= @min_price AND is_verified_by_admin = @is_verified ORDER BY name LIMIT $1 OFFSET $2;

-- name: GetHostelsByCity :many
SELECT * FROM hostels WHERE city = @city ORDER BY name;

-- name: GetHostelsByInstitution :many
SELECT hostels.* FROM hostels INNER JOIN neighborhoods ON hostels.neighborhood_id = neighborhoods.id WHERE neighborhoods.institution_id = @institution_id AND hostels.estimated_price_range >= @min_price AND hostels.estimated_price_range <= @max_price AND is_verified_by_admin = @is_verified LIMIT $1 OFFSET $2;
